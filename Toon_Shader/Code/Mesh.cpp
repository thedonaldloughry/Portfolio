...
void Mesh::Load(Program* p)
{
    ...
    cellTex = new Texture2D("Assets/Shaders/cellShade1.png");  //COMMENT THIS LINE OUT IF NOT CELL SHADING//
...
void Mesh::DrawLines(Program* p, float& lineLength) // PROGRAM* P will be a different program than the rest of the lighting, using unique FS and VS//
{ // Keep in mind, you must have different draw calls for different shaders. combining them causes massive artifacts and slows the program.
    if(!areLinesThere)
        Mesh::LoadOutlines();
    // VERTEX FORMAT SETTINGS //
    p->use();
    p->setUniform("tolerance", tolerance); // this sets the variable for how often lines should be drawn, depends on how curvy a mesh is
    glBindBuffer(GL_ARRAY_BUFFER, linebuff); // linebuff has all of the vertex and normal data from the edge mesh
    ...
    // DRAW //
    glLineWidth(lineLength);
    glDrawArrays(GL_LINES, 0, ic*2);
}

void Mesh::DrawWater(Program* p)
{
    if(!initialized)
        Mesh::Load(p);
    glBindBuffer(GL_ARRAY_BUFFER, vbuff);
    ...
    p->setUniform("diffuse", dc);
    p->setUniform("specular", sc);
    p->setUniform("freq", 1.5);
    p->setUniform("speed", 1.0);
    p->setUniform("steepness", 1.0);
    p->setUniform("amplitude", 0.3);
